...
from javax.swing import JDialog, JLabel, JButton, JComboBox
...
class LoadPipelineDialog(JDialog):
    def __init__(self):
        super(LoadPipelineDialog, self).__init__(MainFrame.get(), "Load Pipeline", True)
        ...
        class LoadAction(ActionListener):
            ...
            def actionPerformed(self, e):
                ...
                SaveManager.loadPipeline(name, pipeline)
                self.parent.setVisible(False)

        self.loadBtn = JButton("Load")
        self.loadBtn.addActionListener(LoadAction(self))
        self.getContentPane().add(self.loadBtn, "6, 2")
        ...
        class DeleteAction(ActionListener):
            ...
            def actionPerformed(self, e):
                ...
                SaveManager.deletePipeline(name)
                self.parent.valueName.removeAllItems()
                for i in SaveManager.pipelines:
                    self.parent.valueName.addItem(i)
                self.parent.valueName.setPrototypeDisplayValue("long text for combo box")

        self.deleteBtn = JButton("Delete")
        self.deleteBtn.addActionListener(DeleteAction(self))
        self.getContentPane().add(self.deleteBtn, "6, 4")
        ...
        self.getContentPane().add(self.cancelBtn, "6, 6")
...
class SaveManager:
    ...
    @staticmethod
    def loadPipeline(name, pipeline):
...
if hasFeeder and hasPipeline:
    LoadPipelineDialog().setVisible(True)